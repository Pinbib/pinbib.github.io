...
import JSR from "@/components/icons/JSR.vue";
...
<template>
  <a :href="url" target="_blank">
    <div class="icons">
      ...
      <div v-else-if="icon==='reddit'">
        <Reddit class="icon"/>
      </div>
      <div v-else-if="icon==='mastodon'">
        <Mastodon class="icon"/>
      </div>
      <div v-else-if="icon==='npm'">
        <NPM class="icon"/>
      </div>
      <div v-else-if="icon==='jsr'">
        <JSR class="icon"/>
      </div>
      <div v-else>
...
<style scoped>
a {
  transition: rotate 2s cubic-bezier(0.01, 0.92, 0.89, 0.5);
}
